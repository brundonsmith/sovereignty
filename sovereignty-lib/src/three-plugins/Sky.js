...
import { Mesh, BoxBufferGeometry, ShaderMaterial, UniformsUtils, BackSide,
	Vector3 } from 'three';
...
const Sky = function(options = {}) {
	...
	uniforms.sunPosition.value.set(
		options.distance * Math.cos( phi ),
		options.distance * Math.sin( phi ) * Math.sin( theta ),
		options.distance * Math.sin( phi ) * Math.cos( theta )
	);
